...
var saveTab = document.getElementById("save");
...
saveTab.addEventListener("click",saveData);
...
function regionTab(areaArr,productArr) {
    ...
    for (var i=0;i<areaData.length;i++){
        ...
        for (var j=0;j<saleData[i].length;j++){
            table += '<td>'+'<input class="Val" style="width: 40px" value='+ saleData[i][j] +'>' +'</td>';
        }
        table += '</tr>'
    }
...
function saveData() {
    ...
    for(var i=0;i<data.length;i++){
        ...
        localStorage.setItem(tabData[i],value);
        // console.log(localStorage.getItem(tabData[i]));
    }

    // console.log(localStorage.tabdata);
}
